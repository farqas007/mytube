...
// Returns recent videos from a specific YouTube channel via the official
// search endpoint (ordered by date), enriched with stats/durations.
async function handleChannelVideos(req, res, params){
  ...
  const maxResults = Math.min(Math.max(parseInt(params.get("max") || "8", 10) || 8, 1), 25);
  const pageToken = (params.get("pageToken") || "").trim();
  const cacheKey = "channelVideos:" + channelId + ":" + maxResults + ":" + pageToken;

  try{
    let url = YT_API_BASE + "/search?part=snippet&type=video&channelId=" +
      encodeURIComponent(channelId) + "&maxResults=" + maxResults + "&order=date";
    if(pageToken){
      url += "&pageToken=" + encodeURIComponent(pageToken);
    }
    const data = await ytFetch(cacheKey, url);
...
async function enrichWithStats(videos){
  ...
  for(const v of videos){
    ...
    if(Number.isFinite(viewCountInt)){
      ...
      v.views = viewCountInt ? normalize.formatCount(viewCountInt) : "";
    }
